...
public class PlayerAvatarController : MonoBehaviour, IHideAvatarAreaHandler, IHidePassportAreaHandler
{
    ...
    private void OnUserProfileOnUpdate(UserProfile profile)
    {
        ...
        Debug.Log("avatarLoadingCts.Token =  " + avatarLoadingCts.Token);
    ...
    {
        Debug.Log("當前的currentAvatar = " + currentAvatar);
        ...
        try
        {
            ...
            if (avatar.status != IAvatar.Status.Loaded || !profile.avatar.HaveSameWearablesAndColors(currentAvatar))
            {
                ...
                foreach (string item in wearableItems)
                {
                    //Debug.Log("用户的wearables的元素 = " + item);
                }
                ...
                Debug.Log("forceRender集合的值:");
                foreach (string item in profile.avatar.forceRender)
                {
                    Debug.Log(item);
                }
        ...
        finally
        {
            ...
            Debug.Log("LoadingAvatarRoutine 的 player = " + player.avatar);
            ...
            onPointerDown.Initialize(           // 更新點擊玩家身上的信息組件面板
                pointerEventModel,
                null,
                player
            );
