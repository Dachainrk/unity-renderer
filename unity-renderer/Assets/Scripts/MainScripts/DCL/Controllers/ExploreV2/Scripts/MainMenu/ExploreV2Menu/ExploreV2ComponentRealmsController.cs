...
public class ExploreV2ComponentRealmsController: IDisposable
{
    ...
    internal void UpdateAvailableRealmsInfo(IEnumerable<RealmModel> currentRealmList)
    {
        ...
        {
            Debug.Log("realm serverName = " + realm.serverName);
            Debug.Log("realm usersCount = " + realm.usersCount);
            Debug.Log("realm url = " + realm.url);
        }
        ...
        if (realmList != null)
        {
            ...
            Debug.Log("UpdateAvailableRealmsInfo servername = " + serverName);
